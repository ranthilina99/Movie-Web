...
export function PremiumPlayer({
    ...
    if (!isPip) {
      if ("pictureInPictureEnabled" in document) {
        video
          ...
          .then(() => setIsPip(true))
          .catch((error) => console.error("Failed to enter Picture-in-Picture mode:", error))
      }
    } else {
      if (document.pictureInPictureElement === video) {
        document
          ...
          .then(() => setIsPip(false))
          .catch((error) => console.error("Failed to exit Picture-in-Picture mode:", error))
      }
    }
  }
  ...
  return (
    ...
    >
      ...
      {!isPlaying && (
        <div className="absolute inset-0 flex items-center justify-center z-10">
          <Button
            variant="ghost"
            size="icon"
            className="h-16 w-16 rounded-full bg-black/50 text-white hover:bg-yellow-600 hover:scale-110 transition-all"
            onClick={handlePlayPause}
          >
      ...
      >
        ...
        <div className="relative group/progress">
          <Slider
            ...
            className="w-full cursor-pointer [&>span:first-child]:h-1 [&>span:first-child]:bg-white/30 [&_[role=slider]]:h-3 [&_[role=slider]]:w-3 [&_[role=slider]]:opacity-0 group-hover/progress:[&_[role=slider]]:opacity-100 [&_[role=slider]]:border-0 [&>span:first-child_span]:bg-yellow-600"
          />
            ...
            <div className="relative group/volume">
              ...
              >
                <Slider
                  ...
                  className="h-24 cursor-pointer [&>span:first-child]:w-1 [&>span:first-child]:bg-white/30 [&_[role=slider]]:h-3 [&_[role=slider]]:w-3 [&_[role=slider]]:border-0 [&>span:first-child_span]:bg-yellow-600"
                />
              </div>
            </div>
      ...
      {/* Premium Player Branding */}
      <div className="absolute top-4 right-16 bg-gradient-to-r from-red-600 to-yellow-500 text-white text-xs px-2 py-0.5 rounded-sm">
        Premium
      </div>
    </div>
  )
